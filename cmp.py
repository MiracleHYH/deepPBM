...
# load  Dataset
imgs = np.load('../Data/real/Video_%03d/BMC2012_%03d.npy' % (vidNumber, vidNumber))
imgs /= 256
...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.efc1 = nn.Linear(h_layer_4 * 20 * 16, h_layer_5)
        ...
        self.dfc2 = nn.Linear(h_layer_5, h_layer_4 * 20 * 16)
        self.ddrop1 = nn.Dropout(p=0.3, inplace=False)
        self.dconv1 = nn.ConvTranspose2d(h_layer_4, h_layer_3, kernel_size=kernel_size, stride=stride, padding=0,
                                         output_padding=0)
        self.dbn1 = nn.BatchNorm2d(h_layer_3, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True)
        self.dconv2 = nn.ConvTranspose2d(h_layer_3, h_layer_2, kernel_size=kernel_size, stride=stride, padding=0,
                                         output_padding=0)
        self.dbn2 = nn.BatchNorm2d(h_layer_2, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True)
        self.dconv3 = nn.ConvTranspose2d(h_layer_2, h_layer_1, kernel_size=kernel_size, stride=stride, padding=0,
                                         output_padding=1)
    ...
    def Encoder(self, x):
        ...
        eh5 = self.relu(self.edrop1(self.efc1(eh4.view(-1, h_layer_4 * 20 * 16))))
        mu_z = self.mu_z(eh5)
    ...
    def Decoder(self, z):
        ...
        dh3 = self.relu(self.dbn1(self.dconv1(dh2.view(-1, h_layer_4, 20, 16))))
